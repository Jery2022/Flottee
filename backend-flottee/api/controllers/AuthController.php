...
use Core\Response; use App\Models\UsersModel; use App\Helpers\JWTHelper; use App\Helpers\AuthMiddleware;

class AuthController { public function handleForm() { session_start();
...
public function login() { session_start();
...
public function getCsrfToken() { 
    session_start(); 
    require_once __DIR__ . '/../core/Csrf.php'; 
    $token = generateCsrfToken(); Response::json(['csrf_token' => $token]);
 }

public function route($uri) { 
    switch ($uri) { 
        case '/api/routes/csrf-token': $this->getCsrfToken(); 
            break; 
        case '/api/routes/auth/handleForm': $this->handleForm(); 
            break; 
        case '/api/routes/auth/login': $this->login(); 
            break; 
        default: Response::error('Route non trouvée', 404); 
    } 
} 
}
